...
export const sendOutlookTestEmail = async ({ recipientEmail }) => {
  try {
    ...
    if (provider !== 'outlook') {
      console.warn(
        '⚠️ Warning: Email is not an Outlook address, but sending anyway'
      )
    }
...
export const sendCustomDomainTest = async ({ recipientEmail }) => {
  try {
    ...
    if (provider === 'custom-domain') {
      ...
      deliveryTips =
        'For custom domains: 1) Check spam/junk folder, 2) Contact domain administrator if not received, 3) May require sender domain authentication (SPF/DKIM/DMARC), 4) Some business domains block external emails by default'
    } else if (provider === '2creative') {
      domainInfo = `2Creative company email detected. This is an internal company domain.`
      deliveryTips =
        'For 2Creative emails: 1) Check company spam policies, 2) Contact IT administrator if issues persist, 3) Ensure sender is whitelisted in company email security'
    } else if (provider === 'educational') {
      domainInfo = `Educational institution domain detected: ${domain}`
      deliveryTips =
        'For educational emails: 1) Very strict spam filtering, 2) May require sender domain whitelisting, 3) Contact IT support, 4) Check institutional email policies'
    } else if (provider === 'government') {
      domainInfo = `Government domain detected: ${domain}`
      deliveryTips =
        'For government emails: 1) Extremely strict security policies, 2) May block all external emails, 3) Contact IT security team, 4) Formal sender authorization may be required'
    }
    ...
    return {
      ...
      timestamp: new Date(),
      deliveryTips:
        'Email delivery failed. Check email configuration and domain settings.',
  ...
  try {
    ...
    const providerInfo = {
      outlook: {
        ...
        deliveryTips:
          'Check spam folder, add to safe senders, may take 2-5 minutes',
      },
      gmail: {
        name: 'Google Gmail',
      ...
      '2creative': {
        ...
        notes:
          'Internal company domain. Should have reliable delivery if configured properly.',
        deliveryTips:
          'Contact IT admin if not received, check company spam policies',
      },
      educational: {
        name: 'Educational Institution',
        notes: 'Educational domains often have strict email security policies.',
        deliveryTips:
          'Check spam folder, contact IT support for sender whitelisting',
      },
      government: {
        name: 'Government Domain',
        notes:
          'Government domains have very strict security and filtering policies.',
        deliveryTips:
          'Contact IT security team, may require formal sender authorization',
      },
      'custom-domain': {
        name: 'Custom Business Domain',
        notes: 'Custom business domain with unknown email configuration.',
        deliveryTips:
          'Check spam folder, contact domain admin, may need sender authentication setup',
      },
      other: {
