...
const ProjectReports = () => {
  ...
  const reports = useMemo(() => {
    ...
    // Allocation Summary
    const allocationSummary = projects.reduce(
      (acc, project) => {
        const activeAllocations = project.allocations.filter((a) => a.isActive)
        const totalHours = activeAllocations.reduce(
    ...
    const updatesSummary = updates.reduce(
      (acc, update) => {
    ...
    const projectProgress = projects.map((project) => {
      ...
      const totalHours = projectUpdates.reduce(
        (sum, u) => sum + (u.hoursWorked || 0),
        0
      )
      const averageHours =
        projectUpdates.length > 0 ? totalHours / projectUpdates.length : 0
    ...
    const teamPerformance = updates.reduce((acc, update) => {
      ...
      acc[userId].statusDistribution[update.status] =
        (acc[userId].statusDistribution[update.status] || 0) + 1
    ...
    Object.values(teamPerformance).forEach((member) => {
      ...
      member.averageHours =
        member.totalUpdates > 0
          ? Math.round((member.totalHours / member.totalUpdates) * 10) / 10
          : 0
    })
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              ...
              <div className="space-y-3">
                {Object.entries(reports.statusSummary).map(
                  ([status, count]) => (
                    <div
                      key={status}
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6">
              ...
              <div className="space-y-3">
                {Object.entries(reports.prioritySummary).map(
                  ([priority, count]) => (
                    <div
                      key={priority}
                      className="flex items-center justify-between"
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {reports.projectProgress.map((project) => (
                    <tr key={project.id}>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-semibold ${getStatusColor(project.status)}`}
                        >
                          {project.status}
                        </span>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <span
                          className={`text-sm font-medium ${
                            project.metrics.blockersCount > 0
                              ? 'text-red-600'
                              : 'text-green-600'
                          }`}
                        >
                  ...
                  >
                    <div className="mb-4 flex items-center justify-between">
                      <div>
                        ...
                        <div className="mt-2 flex items-center space-x-4">
                          <span className="text-sm text-gray-500">
                            {member.totalUpdates} updates •{' '}
                            {Math.round(member.totalHours)}h total
                          </span>
                          <span className="text-sm text-gray-500">
                      ...
                      <div className="flex space-x-2">
                        {Object.entries(member.statusDistribution).map(
                          ([status, count]) => (
                            <span
                              key={status}
