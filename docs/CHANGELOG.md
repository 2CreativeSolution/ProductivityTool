...
## 2026-02-09
- Consolidated assets admin workflow and reports UX: moved create/assign actions into inventory/assignments headers, removed `/assets/management` route/nav entry, migrated asset management modals to shared dialog components with sticky footer actions, hid assign action for non-admins on `/assets/assignments`, standardized employee reports (`/assets/reports/employees` and detail) to shared table patterns, and moved employee-detail CSV export to page header. (evidence: web/src/Routes.jsx, web/src/components/AppSidebar/AppSidebar.jsx, web/src/pages/AssetsInventoryPage/AssetsInventoryPage.jsx, web/src/pages/AssetsAssignmentsPage/AssetsAssignmentsPage.jsx, web/src/components/AssetTracker/AssetManagement.jsx, web/src/components/AssetTracker/AssetAssignmentForm.jsx, web/src/components/AssetTracker/AssetReports.jsx, web/src/pages/AssetsReportsEmployeeDetailPage/AssetsReportsEmployeeDetailPage.jsx, web/src/pages/AssetsManagementPage/AssetsManagementPage.jsx)
- Fixed project edit failure propagation so unsuccessful updates do not continue dialog success/reset flow. (evidence: web/src/components/ProjectTracker/ProjectManagement.jsx)
- Fixed asset tracker employee access after restricting `assets` query to admins: gated the inventory query in shared `AssetTracker` so non-admin views (`/assets/assignments`, `/assets/requests`) no longer trigger `AssetsQuery` Forbidden errors. (evidence: web/src/components/AssetTracker/AssetTracker.jsx, api/src/graphql/assets.sdl.js)
- Standardized asset reports employee experience: `/assets/reports/my` is employee-only (admins redirect to overview), date-range filters refetch the report query, and CSV export downloads current rows. (evidence: web/src/Routes.jsx, web/src/pages/AssetsReportsMyPage/AssetsReportsMyPage.jsx, web/src/components/AssetTracker/AssetReports.jsx, api/src/graphql/assetAssignments.sdl.js, api/src/services/assetAssignments/assetAssignments.js)
- Normalized auth failure semantics for `assetsByUser`: blocked access now throws `ForbiddenError` instead of a generic error so clients can treat it as an expected permission failure. (evidence: api/src/services/assets/assets.js)
- Marked workspace packages with `1.0.0-beta` to reflect current pre-release state (private packages, internal release label). (evidence: package.json, api/package.json, web/package.json)

## 2026-02-07
- Added authenticated settings password-change flow end-to-end: new `/settings/change-password` page and sidebar entry, new `changePassword` GraphQL mutation/input, and form UX with current/new/confirm fields, match validation, and non-blocking strength guidance. (evidence: web/src/pages/User/ChangePasswordPage/ChangePasswordPage.jsx, web/src/components/Settings/ChangePasswordForm/ChangePasswordForm.jsx, web/src/components/AppSidebar/AppSidebar.jsx, web/src/Routes.jsx, api/src/graphql/users.sdl.js, api/src/services/users/users.js)
- Hardened credential update + account email notifications: change-password now verifies current password, regenerates hash and salt on update, and sends password-changed confirmation email. (evidence: api/src/services/users/users.js, api/src/lib/emailService.js)
- Consolidated auth account-email templates and wiring: forgot-password now calls shared email service template, welcome email uses the same template and is always attempted on signup, and signup now assigns default `USER` role. (evidence: api/src/functions/auth.js, api/src/lib/emailService.js)
- Consolidated app-shell and page-header refactor across dashboard/admin/resources/user screens: introduced reusable `AppContentShell` and `PageHeader`, moved `/me` subpages to dedicated routes (`/me/bookings`, `/me/attendance`, `/me/vacation`), moved key page descriptions/actions into headers, standardized header action buttons to small black variant, refreshed sidebar active-state visibility, and aligned primary/secondary CTA usage in booking/attendance/vacation flows. (evidence: web/src/components/AppContentShell/AppContentShell.jsx, web/src/components/PageHeader/PageHeader.jsx, web/src/components/AppSidebar/AppSidebar.jsx, web/src/Routes.jsx, web/src/pages/DashboardPage/DashboardPage.jsx, web/src/pages/MeBookingsPage/MeBookingsPage.jsx, web/src/pages/MeAttendancePage/MeAttendancePage.jsx, web/src/pages/MeVacationPage/MeVacationPage.jsx, web/src/components/Booking/Booking/Booking.jsx, web/src/components/Attendance/Attendance.jsx, web/src/components/AttendanceCard/AttendanceCard.jsx, web/src/components/VacationPlanner/VacationPlanner.jsx, web/src/pages/User/UsersPage/UsersPage.jsx, web/src/pages/OfficeSupply/AdminSupplyRequestsPage/AdminSupplyRequestsPage.jsx, web/src/pages/OfficeSupply/SupplyCategoriesPage/SupplyCategoriesPage.jsx, web/src/components/OfficeSupply/AdminSupplyRequestManager/AdminSupplyRequestManager.jsx, web/src/components/OfficeSupply/CategoryManager/CategoryManager.jsx)

## 2026-02-06
- Refactored admin user management UX: unified `/admin/users*` show/edit/new into account-style UI, migrated users list to shadcn DataTable (sorting, pagination, page-size), refined action styles and table header visuals, added role indicators/warnings, and hardened account save role handling (draft-based payload, non-admin role pills, missing-role auto-heal to USER, persisted-role summary rendering). (evidence: web/src/components/User/UserForm/UserForm.jsx, web/src/components/User/Users/Users.jsx, web/src/components/User/UsersCell/UsersCell.jsx, web/src/components/User/User/User.jsx, web/src/components/User/UserCell/UserCell.jsx, web/src/components/User/NewUser/NewUser.jsx, web/src/components/User/EditUserCell/EditUserCell.jsx, web/src/components/ui/data-table.jsx, web/src/pages/User/NewUserPage/NewUserPage.jsx, web/src/pages/User/EditUserPage/EditUserPage.jsx, api/src/services/users/users.js, api/src/graphql/users.sdl.js)
